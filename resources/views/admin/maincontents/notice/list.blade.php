              ...
              <tbody>
                <?php if(count($rows)>0){ $sl=1; foreach($rows as $row){?>
                  <tr>
                    ...
                    <td><?=date_format(date_create($row->start_date), "M d, Y")?></td>
                    ...
                    <td>
                      <?php if($row->notice_file != ''){?>
                        <a href="<?=env('UPLOADS_URL').'notice/'.$row->notice_file?>" target="_blank" class="badge bg-primary" download>View Notice</a>
                      <?php }?>
                    </td>
                    <td>
