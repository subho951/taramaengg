...
class FaqController extends Controller
{
        ...
        public function add(Request $request){
            ...
            if($request->isMethod('post')){
                ...
                if($this->validate($request, $rules)){
                    $checkValue = Faq::where('question', '=', $postData['question'])->count();
                    if($checkValue <= 0){
                        $fields = [
    ...
    /* edit */
        public function edit(Request $request, $id){
            ...
            if($request->isMethod('post')){
                ...
                if($this->validate($request, $rules)){
                    $checkValue = Faq::where('question', '=', $postData['question'])->where('id', '!=', $id)->count();
                    if($checkValue <= 0){
                        $fields = [
                            'faq_category_id'       => $postData['faq_category_id'],
                            'question'              => $postData['question'],
    ...
    /* change home page status */
    /* module sorting */
        public function sortingContent(Request $request){
          ...
          if(count($values) > 0){
            for($c=0;$c<count($values);$c++){
              DB::table($tableName)
                ->where($primaryKey,$values[$c])
                ->update(['rank' => ($c+1)]);
            }
          }      
          $data           = array("status" => 1, "message" => 'Data Sorted Successfully !!!', "response" => []);
